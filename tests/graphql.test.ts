...
describe("GraphQL gateway", () => {
  ...
  it("lists memories and searches", async () => {
    engine.add("SQLite", "local db", { type: "decision", tags: ["db"] });
    const list = await executeGraphql(engine, {
      query: "{ memories { id title } }",
      variables: { limit: 5 },
    });
    expect((list.data as any).memories.length).toBeGreaterThanOrEqual(1);

    const search = await executeGraphql(engine, {
      query: "{ search { count results { title } } }",
      variables: { q: "sqlite" },
    });
    expect((search.data as any).search.count).toBeGreaterThanOrEqual(1);
  });
