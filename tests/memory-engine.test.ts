...
describe("MemoryEngine", () => {
  ...
  it("adds and lists memories", () => {
    const e = engine.add("Use SQLite", "Local first storage", {
      type: "decision",
    });
  ...
  it("hybrid search returns ranked results", () => {
    engine.add("Auth JWT", "Short lived tokens", {
      type: "security",
      tags: ["auth"],
    });
  ...
  it("deduplicates near-identical content", () => {
    const a = engine.add("Same title", "Identical body content here", {
      type: "note",
    });
    const b = engine.add("Same title", "Identical body content here", {
      type: "note",
    });
