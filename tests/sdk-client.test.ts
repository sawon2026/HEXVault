...
import { HexVaultClient, HexVaultApiError, createClient } from "../packages/sdk/src/index.js";
...
describe("@hexvault/sdk client", () => {
  ...
  it("throws HexVaultApiError on HTTP error", async () => {
    const c = new HexVaultClient({
      fetch: async () => new Response(JSON.stringify({ error: "nope" }), { status: 500 }),
    });
  ...
  it("parses successful JSON", async () => {
    const c = new HexVaultClient({
      fetch: async () =>
        new Response(JSON.stringify({ ok: true, version: "2.0.0" }), { status: 200 }),
    });
