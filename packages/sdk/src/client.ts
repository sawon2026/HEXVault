...
export class HexVaultClient {
  ...
  constructor(opts: HexVaultClientOptions = {}) {
    ...
    this.token = opts.token ?? (typeof process !== "undefined" ? process.env.HEXVAULT_API_TOKEN : undefined);
  ...
  private async request<T>(method: string, path: string, body?: unknown): Promise<T> {
    ...
    try {
      ...
      if (!res.ok) {
        throw new HexVaultApiError(`HEXVault API ${method} ${path} → ${res.status}`, res.status, data);
      }
      return data as T;
    } finally {
      clearTimeout(timer);
  ...
  listMemories(opts?: { limit?: number; type?: MemoryType }): Promise<{ count: number; items: Memory[] }> {
  ...
  search(query: string, limit = 10): Promise<{ query: string; count: number; results: SearchHit[] }> {
  ...
  chat(question: string, context?: string): Promise<ChatResult> {
    return this.request("POST", "/v1/chat", { question, context });
  }
  ...
  review(opts: { title?: string; body?: string; diff?: string }): Promise<ReviewResult> {
  ...
  graph(opts?: { limit?: number; w?: number; h?: number }): Promise<KnowledgeGraph> {
    const q = new URLSearchParams();
    if (opts?.limit) q.set("limit", String(opts.limit));
