 ...
 * No full AST required; works on TS/JS/JSX source text.
 */
import fs from "fs";
...
function scanDeadHints(file: string, text: string): DeadCodeHint[] {
  ...
  // Naive unused export: export function/const never referenced again in same file
  const exportRe =
    /export\s+(?:async\s+)?(?:function|const|class|let)\s+([A-Za-z_$][\w$]*)/g;
  let m: RegExpExecArray | null;
  while ((m = exportRe.exec(text))) {
    const name = m[1];
    if (name === "default") continue;
    const re = new RegExp(`\\b${name}\\b`, "g");
    const refs = countMatches(text, re);
    // 1 = the export declaration itself
    if (refs <= 1) {
      ...
      hints.push({
        ...
        kind: "unused-export",
        line,
        detail: `Exported "${name}" not referenced in this file (may still be used elsewhere)`,
      });
    }
  }
...
export interface AnalyzeOptions {
  cwd?: string;
  globs?: string[];
  /** Max files to score in hotspots list */
  topN?: number;
}
...
export async function analyzeProject(
  ...
  const patterns = opts.globs || DEFAULT_GLOBS;
  ...
  for (const abs of files) {
    ...
    if (text.length > 500_000) continue; // skip huge generated files
  ...
  const avgScore =
    complexity.length === 0
      ? 0
      : Math.round(
          complexity.reduce((s, c) => s + c.score, 0) / complexity.length,
        );
  const maxScore = complexity[0]?.score ?? 0;
