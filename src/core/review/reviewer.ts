  ...
  async review(
    ...
    if (this.options.checks?.includes("security")) {
      const securityPatterns = [
        ...
        { pattern: /secret\s*=\s*['"][^'"]+['"]/i, msg: "Possible hardcoded secret" },
        { pattern: /eval\s*\(/i, msg: "Use of eval() is dangerous" },
        { pattern: /innerHTML\s*=/i, msg: "innerHTML usage can lead to XSS" },
        { pattern: /dangerouslySetInnerHTML/i, msg: "dangerouslySetInnerHTML detected" },
      ];
    ...
    if (this.options.checks?.includes("consistency") && relatedMemories.length > 0) {
