 ...
 * Export vault → JSON bundle; import/merge into local engine.
 */
...
/** Export current vault to a portable JSON bundle */
...
/** Parse and validate a sync bundle from unknown JSON */
...
/**
 * Import memories. Skips near-duplicates via engine dedup when available;
 * otherwise skips if same id already exists.
 */
export function importBundle(
  ...
  return {
    added,
    skipped,
    totalInBundle: bundle.memories.length,
  };
}

/** Merge helper used by HTTP pull-from-remote flows */
export async function pullFromRemote(
  ...
  if (!res.ok) {
    throw new Error(`Remote export failed: ${res.status}`);
  }
  const json = await res.json();
  const bundle = parseBundle(json);
  return importBundle(engine, bundle);
}
