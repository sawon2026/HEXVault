 ...
 * Nodes: memory | type | tag
 * Edges: memory→type, memory→tag, tag co-occurrence (optional)
 */
...
export function buildKnowledgeGraph(
  ...
  // Lightweight related edges: same type memories share a soft link via type already;
  // add tag co-occurrence between memories that share a tag (cap)
...
export function layoutGraph(
  ...
  return {
    nodes: nodes.map(({ vx: _v, vy: _y, ...rest }) => rest),
    edges: graph.edges,
  };
}
