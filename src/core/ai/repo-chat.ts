...
function ruleAnswer(
  ...
  const lines = memories.slice(0, 5).map(
    (m, i) => `${i + 1}. [${m.type}] ${m.title}\n   ${m.content.slice(0, 200)}`
  );
  return (
    `Based on ${memories.length} project memory(ies):\n\n` +
    lines.join("\n\n") +
    `\n\n(Rule-based answer — set API keys for richer LLM replies.)`
  );
}
...
export async function repoChat(opts: ChatOptions): Promise<ChatResult> {
  ...
  const messages: LLMMessage[] = [
    {
      role: "system",
      content:
        "You are HEXVault, a project-memory assistant. Answer using the provided project memories. Cite memory numbers like [1]. If memories are insufficient, say so. Be concise and practical.",
    },
  ];
  ...
  try {
    const res = await registry.complete(messages, { maxTokens: 600, temperature: 0.3 });
    if (res.content.trim().length > 10) {
