...
export class GitLabClient {
  ...
  async getMergeRequest(iid: number): Promise<GitLabMR> {
    return this.request(`/projects/${this.projectId}/merge_requests/${iid}`) as Promise<GitLabMR>;
  }

  async postNote(iid: number, body: string) {
    return this.request(`/projects/${this.projectId}/merge_requests/${iid}/notes`, "POST", {
      body,
    });
  }
