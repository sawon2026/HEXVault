...
function banner() {
  ...
  ╔════════════════════════════════════╗
  ║       HEXVault Interactive TUI        ║
  ╚════════════════════════════════════╝
`));
  console.log(chalk.gray("  Project memory · type a number + Enter\n"));
}
...
function engine(): MemoryEngine {
  ...
  return new MemoryEngine({ dbPath: path.resolve(config.memory.path) });
}
...
async function searchMemories(iface: readline.Interface) {
  ...
  console.log(chalk.bold(`\n  ${hits.length} results\n`));
  for (const h of hits) {
    console.log(`  ${chalk.cyan(h.rankScore.toFixed(2))} ${chalk.magenta(`[${h.entry.type}]`)} ${h.entry.title}`);
...
async function addMemory(iface: readline.Interface) {
  ...
  if (!content) return;
  const type =
    (await ask(iface, chalk.cyan("  Type [note/decision/bugfix/architecture/security] › "))) ||
    "note";
  const tagsRaw = await ask(iface, chalk.cyan("  Tags (comma) › "));
  const tags = tagsRaw ? tagsRaw.split(",").map((t) => t.trim()).filter(Boolean) : [];
  const e = engine();
  const entry = e.add(content.slice(0, 60), content, { type: type as never, tags, source: "tui" });
  e.close();
    ...
    try {
      switch (choice) {
        ...
        case "":
          ...
          break;
        default:
          console.log(chalk.yellow("  Unknown — Enter for menu, 0 to exit\n"));
          menu();
      }
    } catch (err) {
